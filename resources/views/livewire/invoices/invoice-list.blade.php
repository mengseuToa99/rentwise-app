<div class="min-h-screen bg-gray-50 dark:bg-zinc-950 py-4">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <button
                    ...
                    type="button"
                    class="inline-flex items-center whitespace-nowrap px-3 py-2 bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-md font-medium text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-zinc-700 transition-colors"
                >
                    @if($displayMode === 'card')
                ...
                @if($viewMode !== 'tenant')
                    <a href="{{ route('simple-mode.home') }}" class="inline-flex items-center whitespace-nowrap px-3 py-2 bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-zinc-200 rounded-md font-medium text-sm text-white dark:text-zinc-900 shadow-sm transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h10M4 18h7" />
                        </svg>
                        Simple Mode
                    </a>
                    <a href="{{ route('invoices.create') }}" class="inline-flex items-center whitespace-nowrap px-3 py-2 bg-blue-600 dark:bg-blue-700 hover:bg-blue-700 dark:hover:bg-blue-600 rounded-md font-medium text-sm text-white shadow-sm transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                        </svg>
                        Add Invoice
                    </a>
                    <a href="{{ route('invoices.quick') }}" class="inline-flex items-center whitespace-nowrap px-3 py-2 bg-amber-600 hover:bg-amber-700 rounded-md font-medium text-sm text-white shadow-sm transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                        Quick Invoice
                    </a>
                    <a href="{{ route('invoices.batch') }}" class="inline-flex items-center whitespace-nowrap px-3 py-2 bg-green-600 dark:bg-green-700 hover:bg-green-700 dark:hover:bg-green-600 rounded-md font-medium text-sm text-white shadow-sm transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @else
            @if($displayMode === 'card')
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 min-h-[calc(100vh-2rem)]">
                    @foreach ($invoices as $invoice)
                        <div wire:key="invoice-{{ $invoice->invoice_id }}" class="perspective h-full">
                            <div class="relative w-full h-full transform transition-all duration-700 ease-in-out" 
                                ...
                                <div class="absolute w-full bg-white dark:bg-zinc-900 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300 backface-hidden border border-gray-200 dark:border-zinc-800">
                                                    ...
                                                    <div class="grid grid-cols-2 gap-2">
                                                        @foreach($invoice->utility_readings as $reading)
                                                            <div class="bg-gray-50 dark:bg-zinc-800 rounded-md p-2">
                                                                <div class="flex justify-between items-center">
                                                                    <span class="text-xs font-medium text-gray-900 dark:text-white">{{ $reading->utility_name }}</span>
                                                                    <span class="text-xs text-gray-600 dark:text-gray-400">{{ $reading->usage_amount }} units</span>
                                                                </div>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                ...
                                <div class="absolute w-full bg-white dark:bg-zinc-900 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300 backface-hidden rotate-y-180 border border-gray-200 dark:border-zinc-800">
                                    <div class="relative flex flex-col">
                                        ...
                                        <div class="p-4 space-y-3">
                                            ...
                                            @if(isset($invoice->utility_readings) && count($invoice->utility_readings) > 0)
                                                <div class="space-y-2">
                                                    @foreach($invoice->utility_readings as $reading)
                                                        <div class="border border-gray-200 dark:border-zinc-700 rounded-md p-2">
                                            ...
                                            <div class="border border-gray-200 dark:border-zinc-700 rounded-md p-2">
                                                <div class="space-y-1 text-xs">
                                                    ...
                                                    <div class="flex justify-between font-semibold pt-1 border-t border-gray-200 dark:border-zinc-700">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-800 px-4 py-3 flex justify-between items-center border-t border-gray-200 dark:border-zinc-700">
                                            <button 
                                                @click="isFlipped = !isFlipped" 
                        ...
                        <div class="inline-block min-w-full align-middle">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-zinc-700">
                                <thead class="bg-gray-50 dark:bg-zinc-800">
                                    <tr>
                                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">Invoice #</th>
