        ...
        @endif
                ...
                <form wire:submit="save" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Property Selection -->
                        <div>
                            <x-input-label for="selectedProperty" :value="__('maintenance.form.property')" />
                            <select wire:model.live="selectedProperty" @if($mode === 'show') disabled @endif class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-600 dark:bg-zinc-700 focus:border-indigo-500 focus:ring-indigo-500 dark:text-white">
                                <option value="">{{ __('maintenance.select.select_property') }}</option>
                        ...
                        <!-- Unit Selection -->
                        <div>
                            <x-input-label for="selectedUnit" :value="__('maintenance.form.room')" />
                            <select wire:model="selectedUnit" @if($mode === 'show') disabled @endif class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-600 dark:bg-zinc-700 focus:border-indigo-500 focus:ring-indigo-500 dark:text-white">
                                <option value="">{{ __('maintenance.select.select_room') }}</option>
                                @foreach($units as $unit)
                                    <option value="{{ $unit->room_id }}">{{ __('maintenance.select.room_label') }} {{ $unit->room_number }} - {{ $unit->room_name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mt-4">
                        ...
                        <input 
                            ...
                            placeholder="{{ __('maintenance.placeholders.title') }}"
                    ...
                    @if($isLandlord || $mode === 'show')
                        <!-- Status -->
                    <div>
                                <x-input-label for="status" :value="__('maintenance.form.status')" />
                    ...
                    <div class="mt-4">
                        ...
                        <textarea 
                            ...
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-600 dark:bg-zinc-700 focus:border-indigo-500 focus:ring-indigo-500 dark:text-white" 
                            rows="4"
                            placeholder="{{ __('maintenance.placeholders.description') }}"
                        ></textarea>
                    ...
                    @if($isLandlord || $mode === 'show')
                        <!-- Landlord Notes -->
                        <div class="mt-4">
                            <x-input-label for="landlord_notes" :value="__('maintenance.form.landlord_notes')" />
                            <textarea 
                                id="landlord_notes"
                                ...
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-600 dark:bg-zinc-700 focus:border-indigo-500 focus:ring-indigo-500 dark:text-white" 
                                rows="4"
                                placeholder="{{ __('maintenance.placeholders.request_notes') }}"
                            ></textarea>
                                ...
                                @foreach($existingPhotos as $photo)
                                    <div class="relative">
                                        @if(Storage::exists($photo->photo_path))
                                            <img src="{{ asset('storage/' . $photo->photo_path) }}" alt="{{ __('maintenance.photos.maintenance_photo') }}" class="w-full h-48 object-cover rounded-lg shadow-md border-2 border-white dark:border-zinc-800">
                                        @else
                                            <div class="w-full h-48 flex items-center justify-center bg-gray-100 dark:bg-zinc-700 rounded-lg shadow-md border-2 border-white dark:border-zinc-800">
                                                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('maintenance.photos.image_not_found') }}</span>
                                            </div>
                                        @endif
                                        <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-xs p-2 rounded-b-lg">
                                            <p>{{ __('maintenance.photos.type.' . $photo->photo_type) }} {{ __('maintenance.photos.photo') }}</p>
                                            <p>{{ __('maintenance.photos.by') }}: {{ __('maintenance.photos.uploaded_by.' . $photo->uploaded_by_type) }}</p>
                                            <p>{{ $photo->created_at->translatedFormat('M d, Y H:i') }}</p>
                                        </div>
                        ...
                        @if($mode !== 'show')
                            <div class="mt-4">
                                ...
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    @if($isLandlord)
                                        {{ __('maintenance.photos.upload_after') }}
                                    @else
                                        {{ __('maintenance.photos.upload_issue') }}
                                    @endif
                                </p>
                                        ...
                                        @foreach($photos as $index => $photo)
                                            <div class="relative">
                                                @if($photo->temporaryUrl())
                                                    <img src="{{ $photo->temporaryUrl() }}" alt="{{ __('maintenance.photos.temporary_photo') }}" class="w-full h-48 object-cover rounded-lg shadow-md border-2 border-white dark:border-zinc-800">
                    ...
                    @if($mode !== 'show')
                        ...
                        {{ __('maintenance.cancel') }}
                            ...
                            <button type="submit" class="px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ $mode === 'create' ? __('maintenance.submit') : __('maintenance.actions.save_changes') }}
                    </button>
                </div>
                    @else
                        <div class="flex justify-end space-x-3 mt-6">
