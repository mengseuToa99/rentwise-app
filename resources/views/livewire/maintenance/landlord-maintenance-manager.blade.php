<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-8">
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-md p-6">
        <div class="mb-6">
            <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ __('maintenance.titles.request_details') }}</h2>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('maintenance.descriptions.manage_request') }}</p>
        </div>
        ...
        <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('maintenance.sections.property_details') }}</h3>
                <div class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    <p><strong>{{ __('maintenance.columns.property') }}:</strong> {{ $maintenanceRequest->property->property_name }}</p>
                    <p><strong>{{ __('maintenance.columns.unit') }}:</strong> {{ $maintenanceRequest->room->room_number }}</p>
                </div>
            </div>
            <div>
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('maintenance.sections.request_information') }}</h3>
                <div class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    <p><strong>{{ __('maintenance.columns.title') }}:</strong> {{ $maintenanceRequest->title }}</p>
                    <p>
                        <strong>{{ __('maintenance.columns.priority') }}:</strong>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                            ...
                            @endswitch
                        ">
                            {{ __('maintenance.priority.' . $maintenanceRequest->priority) }}
                        </span>
                    </p>
                    <p><strong>{{ __('maintenance.sections.submitted') }}:</strong> {{ $maintenanceRequest->created_at->translatedFormat('M d, Y H:i') }}</p>
                </div>
            </div>
        </div>

        <div class="mb-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('maintenance.form.description') }}</h3>
        ...
        <div class="mb-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-3">{{ __('maintenance.sections.quick_actions') }}</h3>
            <div class="flex space-x-3">
                <button wire:click="quickAction('in_progress')" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                    ...
                    {{ __('maintenance.actions.accept_request') }}
                </button>
                <button wire:click="quickAction('rejected')" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                    ...
                    {{ __('maintenance.actions.reject_request') }}
                </button>
            </div>
        </div>
        @endif
        ...
        <form wire:submit.prevent="updateStatus" class="space-y-6">
            <!-- Status Update -->
            <div>
                <label for="status" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('maintenance.columns.status') }}</label>
                <div class="mt-1">
            ...
            <!-- Landlord Notes -->
            <div>
                <label for="landlord_notes" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('maintenance.sections.notes') }}</label>
                <div class="mt-1">
                    <textarea 
                        id="landlord_notes" 
                        ...
                        placeholder="{{ __('maintenance.placeholders.maintenance_work_notes') }}"
                    ></textarea>
                </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('maintenance.index') }}" class="inline-flex justify-center rounded-md border border-gray-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 shadow-sm hover:bg-gray-50 dark:hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                    {{ __('maintenance.actions.back_to_list') }}
                </a>
                <button type="submit" class="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                    {{ __('maintenance.actions.update_status') }}
                </button>
            </div>
        </form>
    </div>
